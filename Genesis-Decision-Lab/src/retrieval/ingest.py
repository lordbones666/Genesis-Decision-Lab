...
def _base_dir(paths: list[Path]) -> Path | None:
    ...
    return Path(os.path.commonpath([str(path) for path in resolved_parents]))


def _validated_paths(paths: list[Path]) -> list[Path]:
    validated: list[Path] = []
    for path in paths:
        if not path.exists():
            raise FileNotFoundError(f"ingest path does not exist: {path}")
        if not path.is_file():
            raise IsADirectoryError(f"ingest path is not a file: {path}")
        validated.append(path)
    return sorted(validated, key=lambda item: item.resolve().as_posix())
...
def ingest_paths(
    ...
    validated_paths = _validated_paths(paths)
    records: list[IngestedChunk] = []
    common_base_dir = _base_dir(validated_paths)
