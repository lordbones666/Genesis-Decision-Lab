...
EXPECTED_TOOL_OUTPUT_FIELDS = {
    ...
    "network_contagion": {"node_states", "cascade_likelihood", "critical_nodes"},
    "actor_response": {"action_probabilities", "strategic_branches", "adversarial_risk"},
    "hazard_failure": {"hazard_rate", "survival_probability", "event_probability"},
    "scenario_tree_generator": {"branches"},
    "hidden_state_estimator": {"state_probabilities", "most_likely_state", "confidence_band"},
    "uncertainty_decomposition": {"drivers", "dominant_driver", "concentration_index"},
    "value_of_information": {"ranked_candidates", "best_next_signal"},
    "model_arbitration": {
        "combined_estimate",
        "disagreement_score",
        "model_weights",
...
def _construct_mapping(loader: _UniqueKeyLoader, node: yaml.nodes.MappingNode) -> dict[Any, Any]:
    ...
    for key_node, value_node in node.value:
        key = any_loader.construct_object(key_node)
        if key in mapping:
            raise ValueError(f"duplicate key in manifest: {key}")
        mapping[key] = any_loader.construct_object(value_node)
    return mapping
